...
local function teleportBehindPlayer(player)
    ...
    -- Debug: In ra máu hiện tại
    print("My health:", myHealthPercent, "%")
    
    if myHealthPercent < 40 then
        -- Bay lên đầu mục tiêu 300 stud khi máu dưới 40%
        print("SAFE MODE: Flying above target!")
...
local function isTargetValid(player)
    ...
    -- Skip nếu máu target <= 1 (thay vì 1%)
    if targetHumanoid.Health <= 1 then
        print("Skipping target with low health:", targetHumanoid.Health)
        return false
    end
