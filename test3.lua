...
local STUCK_TARGET_TIMEOUT = 5
local WEAPON_NAME = "PhongLon"
local EquipArgs = {"eue", WEAPON_NAME}
...
local function isTargetValid(player)
    return player and player ~= LocalPlayer and player.Character and player.Character:FindFirstChildOfClass("Humanoid") and player.Character.Humanoid.Health > 0
end
    ...
    while true do
        ...
        for _, targetPlayer in ipairs(allPlayers) do
            ...
            if isTargetValid(targetPlayer) then
                ...
                local initialHealth = targetPlayer.Character.Humanoid.Health
                local engagementStartTime = tick()

                while isTargetValid(targetPlayer) do
                    if tick() - engagementStartTime > STUCK_TARGET_TIMEOUT then
                        if targetPlayer.Character.Humanoid.Health >= initialHealth then
                            break 
                        else
                            initialHealth = targetPlayer.Character.Humanoid.Health
                            engagementStartTime = tick()
                        end
                    end
