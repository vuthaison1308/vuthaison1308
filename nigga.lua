...
local function parseSeedValue(seedValue)
    local seedStr = tostring(seedValue)
    if seedStr:find("[Kk]") then
        return tonumber(seedStr:gsub("[Kk]", "")) * 1000
    else
        return tonumber(seedStr:gsub(",", ""))
    end
end
...
task.spawn(function()
    while true do
        if game.PlaceId == 108533757090220 then
            ...
            local SeedHave = parseSeedValue(game:GetService("Players").LocalPlayer.leaderstats.Seeds.Value)
            if (not Have and SeedHave > 2000) then
                print('Nothing')
            else
                task.wait(300)
                hopServer()
...
local function main()
    if game.PlaceId == 108533757090220 then
        ...
        while true do
            ...
            if (not Have and SeedHave > 400) then
                StartRolls = true
                Roll()
            elseif Have and not CheckBackPack() then
                ReturnForLobby()
            elseif Have or SeedHave < 400 then
                local args = {
                    "unique_1",
                    true
                }
        ...
        while true do
            ...
            local currentWins = game:GetService("Players").LocalPlayer.PlayerGui.GameGui.Screen.Middle.Stats.Items.Frame.ScrollingFrame.GamesWon.Items.Items.Val
            local SeedValue = parseSeedValue(game:GetService("Players").LocalPlayer.leaderstats.Seeds.Value)
            
            if tonumber(currentWins.Text) >= 25 then
                if workspace:GetAttribute("MapId") == "map_farm" then
                    game:shutdown()
                    break
                ...
                if workspace:GetAttribute("MapId") == "map_farm" and CheckBackPack() and tonumber(currentWins.Text) < 25 then
                    print('PlayWin')
                    setfpscap(15)
                    PlayWin()
                elseif SeedValue < 2000 and not CheckBackPack() then
                    print('PlayLose')
                    setfpscap(8)
                    PlayLose()
                elseif SeedValue >= 2000 and not CheckBackPack() then
                    game:shutdown()
                    break
                elseif tonumber(currentWins.Text) >= 25 then
                    game:shutdown()
                    break
                end
            end
...
local function runScript()
    ...
    while errorCount < maxRetries do
